...
class WPBakeryShortCode_multimedia_carousel_playlist_item extends WPBakeryShortCode_VC_Tab {
	...
	public function contentAdmin( $atts, $content = null ) {
		...
		for ( $i = 0; $i < count( $width ); $i ++ ) {
			$lbg_my_title.='<%= params.title %>';
			$lbg_my_title=preg_replace('/[^A-Za-z0-9\-]/', '', $lbg_my_title);
			$output .= '<div class="group wpb_sortable multimedia_carousel_playlist_item_class">';
			$output .= '<h3><span class="tab-label" style="padding-left:15px;"><%= params.title %></span> &nbsp; <%= params.youtube %> </h3>     <div data-element_type="vc_playlist_item" class="vc_playlist_item">';
