...
if ( ! class_exists( 'Boombox_Loop_Helper' ) ) {
	...
	class Boombox_Loop_Helper {
		...
		protected static function is_time_for_product() {
			return static::$is_product_enabled &&
			isset( static::$page_layout[ static::$loop_index ] ) &&
		...
		public static function the_post() {
			...
			if ( static::is_time_for_adv() ) {
				do_action( 'boombox/loop-helper/adv', static::$instead_adv, $wp_query, static::$current_page, static::$loop_index );
				$is_adv = true;
			} elseif ( static::is_time_for_newsletter() ) {
				do_action( 'boombox/loop-helper/adv', static::$instead_newsletter, $wp_query, static::$current_page, static::$loop_index );
				$is_newsletter = true;
			} elseif ( static::is_time_for_product() ) {
				do_action( 'boombox/loop-helper/product', static::$instead_products, $wp_query, static::$current_page, static::$loop_index );
				$is_product = true;
			} else {
				the_post();
			}
			...
			return array(
				'is_inject'     => $is_adv || $is_newsletter || $is_product,
				'is_adv'        => $is_adv,
