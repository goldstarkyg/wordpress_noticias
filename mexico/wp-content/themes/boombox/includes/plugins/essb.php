<?php
/**
 * Buddypress plugin functions
 *
 * @package BoomBox_Theme
 */

// Prevent direct script access.
if ( ! defined( 'ABSPATH' ) ) {
    die( 'No direct script access allowed' );
}


if ( ! class_exists( 'Boombox_Essb' ) ) {

    class Boombox_Essb
    {

        private $display_position_name = 'display-41';
        private $position_name = 'boombox';
        private $rendering = false;
        private $options;

        /**
         * Singleton.
         */
        static function get_instance()
        {
            static $Inst = null;
            if ($Inst == null) {
                $Inst = new self();
            }

            return $Inst;
        }

        /**
         * Constructor
         */
        function __construct()
        {
            if( ! boombox_is_plugin_active( 'easy-social-share-buttons3/easy-social-share-buttons3.php' ) ) {
                return;
            }

            $this->options = get_option(ESSB3_OPTIONS_NAME);
            $this->hooks();
        }

        /**
         * Setup Hooks
         */
        private function hooks()
        {
            add_action( 'init', array( $this, 'assign_positions' ), 99 );

            add_filter( 'essb4_custom_method_list', array( $this, 'register_position' ), 10, 1 );
            add_filter( 'essb4_custom_positions', array( $this, 'register_display_position' ), 10, 1 );
            add_filter( 'essb_is_theme_integrated', array( $this, 'essb_theme_integrated' ), 10 );
            add_filter( 'essb4_button_positions', array( $this, 'position_options' ), 10, 1 );
            add_filter( 'essb4_button_positions_mobile', array( $this, 'position_options' ), 10, 1);
            add_filter( 'essb4_position_style_boombox', array( $this, 'boombox_essb_position_style_boombox' ), 10, 1 );
            add_filter( 'essb4_get_cached_counters', array($this, 'boombox_essb_get_cached_counters'), 10, 1 );
            add_filter( 'essb_opengraph_desc', array( $this, 'fix_user_og_description' ), 10, 1 );

            add_filter( 'boombox/customizer/settings_rating', array($this, 'boombox_essb_settings_rating' ), 10, 3 );
            add_filter( 'boombox/customizer_default_values', array($this, 'boombox_essb_customizer_default_values' ), 10, 1 );
            add_filter( 'boombox/fake_share_count', array($this, 'boombox_essb_fake_share_count'), 10, 1 );
            add_filter( 'boombox/shares_meta_key', array($this, 'boombox_essb_shares_meta_key'), 10, 1 );
            add_filter( 'boombox/essb/default_options', array( $this, 'boombox_essb_default_options' ), 10, 1 );
            add_filter( 'mashsb_opengraph_meta', array( $this, 'remove_mashshare_meta_tags' ), 999, 1 );
            add_filter( 'mashsb_twittercard_meta', array( $this, 'remove_mashshare_meta_tags' ), 999, 1 );
        }

        /**
         * Remove activation notice and make plugin to be integrated from theme
         * @return bool
         */
        public function essb_theme_integrated() {
            return true;
        }

        /**
         * Check if is rendering buttons
         *
         * @return bool
         */
        public function is_rendering()
        {
            return (bool)$this->rendering;
        }

        /**
         * Get plugin options
         *
         * @return mixed|void
         */
        public function get_options()
        {
            return $this->options;
        }

        /**
         * Setup button
         *
         * @param $button_style
         * @return mixed
         */
        public function boombox_essb_position_style_boombox($button_style)
        {
            if ($this->is_rendering()) {
                $button_style['show_counter'] = in_array('share_count', boombox_get_theme_option('layout_post_share_box_elements'));
            }
            return $button_style;
        }

        /**
         * Plugin activation callback overwrite
         */
        public function install()
        {
            $mail_salt_check = get_option(ESSB3_MAIL_SALT);
            if (!$mail_salt_check || empty($mail_salt_check)) {
                $new_salt = mt_rand();
                update_option(ESSB3_MAIL_SALT, $new_salt);
            }

            $exist_settings = get_option(ESSB3_OPTIONS_NAME);
            if (!$exist_settings) {
                $default_options = '';

                $options = json_decode( base64_decode( $default_options ), true );
                $options = apply_filters( 'boombox/essb/default_options', $options );
                $default_options = base64_encode( json_encode( $options ) );

                // set that we run plugin for first time
                update_option(ESSB3_FIRST_TIME_NAME, 'false');
                $options_base = ESSB_Manager::convert_ready_made_option($default_options);
                if ($options_base) {
                    update_option(ESSB3_OPTIONS_NAME, $options_base);
                }
            }

            // clear stored add-ons on activation of plugin
            delete_option('essb3_addons');

        }

        /**
         * Register theme positions
         *
         * @param $methods
         * @return mixed
         */
        public function register_position($methods)
        {
            $methods[$this->display_position_name] = __('Boombox', 'essb');

            return $methods;
        }

        /**
         * Register theme display positions
         *
         * @param $positions
         * @return mixed
         */
        public function register_display_position($positions)
        {
            $positions[$this->position_name] = __('Boombox', 'essb');

            return $positions;
        }

        /**
         * Assign positions layouts
         *
         * @param $positions
         * @return mixed
         */
        public function position_options($positions)
        {
            $positions[$this->position_name] = array(
                'image' => 'assets/images/display-positions-09.png',
                'label' => __('Boombox Position', 'essb')
            );

            return $positions;
        }

        /**
         * Assign display positions to appropriate positions
         */
        public function assign_positions()
        {
            if (is_admin()) {
                if (class_exists('ESSBOptionsStructureHelper')) {
                    essb_prepare_location_advanced_customization('where', $this->display_position_name, $this->position_name);
                }

            }
        }

        /**
         * Render theme position
         */
        public function render()
        {
            $general_options = essb_core()->get_general_options();

            if (is_array($general_options)) {
                if (in_array($this->position_name, $general_options['button_position'])) {
                    $this->rendering = true;
                    echo essb_core()->generate_share_buttons($this->position_name);
                    $this->rendering = false;
                }
            }
        }

        /**
         * Adds fake count to total counter and each social network
         *
         * @param $counters
         * @return mixed
         */
        public function boombox_essb_get_cached_counters($counters)
        {

            $fake_count = boombox_post_shares_count($counters['total'], get_the_ID());

            if ($fake_count > $counters['total']) {
                $fake_constant = $fake_count - $counters['total'];

                $social_counters = array_diff_key($counters, array_flip(array('total')));
                $social_counters_count = count($social_counters);
                $most_shared = max($social_counters);
                $least_shared = min($social_counters);
                $least_shared = $least_shared ? $least_shared : 1;

                $i = 1;
                foreach ($social_counters as $network_name => $count) {
                    if (abs($count) == 0) {
                        $asd = max(($most_shared - $least_shared - $i), 0);
                        $social_counters[$network_name] = $i;
                        $counters['total'] += $social_counters[$network_name];
                    }
                    $i++;
                }

                $i = 1;
                $total = 0;
                foreach ($social_counters as $network_name => $count) {
                    if ($i == $social_counters_count) {
                        $counters[$network_name] = $fake_count - $total;
                    } else {
                        $counters[$network_name] = floor(($count / $counters['total']) * $fake_count);
                        $total += $counters[$network_name];
                    }
                    $i++;
                }

                $counters['total'] = $fake_count;
            }

            return $counters;
        }

        /**
         * Fix og:description meta for user profile
         * @param $og_description
         * @return string
         */
        function fix_user_og_description( $og_description ) {
            if( is_plugin_active( 'buddypress/bp-loader.php' ) && function_exists( 'bp_is_user' ) && bp_is_user() ) {
                $og_description = boombox_get_user_meta_description( bp_displayed_user_id() );
            }
            return $og_description;
        }

        /**
         * Add customizer control
         *
         * @param $settings
         * @param $option_name
         * @param $customizer_defaults
         * @return mixed
         */
        public function boombox_essb_settings_rating($settings, $option_name, $customizer_defaults)
        {
            // Fake views count
            $settings['fields'][] = array(
                'setting' => array(
                    'id' => $option_name . '[settings_rating_essb_fake_share_count]',
                    'args' => array(
                        'default' => $customizer_defaults['settings_rating_essb_fake_share_count'],
                        'type' => 'option',
                        'capability' => 'edit_theme_options',
                        'sanitize_callback' => 'sanitize_text_field',
                    )
                ),
                'control' => array(
                    'id' => 'boombox_settings_rating_essb_fake_share_count',
                    'args' => array(
                        'label' => esc_html__('"ESSB" Fake Share Count', 'boombox'),
                        'section' => 'boombox_settings_rating_panel',
                        'settings' => $option_name . '[settings_rating_essb_fake_share_count]',
                        'type' => 'number',
                        'priority' => 201
                    )
                )
            );

            return $settings;
        }

        /**
         * Setup customizer default value
         *
         * @param $values
         * @return mixed
         */
        public function boombox_essb_customizer_default_values($values)
        {
            $values['settings_rating_essb_fake_share_count'] = 0;

            return $values;
        }

        /**
         * Add fake count
         * @param $fake_count
         * @return int
         */
        public function boombox_essb_fake_share_count($fake_count)
        {
            return boombox_get_theme_option('settings_rating_essb_fake_share_count');
        }

        /**
         * Use ESSB meta key for sharing calculation
         *
         * @param $meta_key
         * @return string
         */
        public function boombox_essb_shares_meta_key($meta_key)
        {
            $meta_key = 'essb_c_total';

            return $meta_key;
        }

        /**
         * Overwrite default options for theme
         * @param $options
         * @return array
         */
        public function boombox_essb_default_options( $options ) {

            $boombox_options = array(
                'display_in_types' => array(
                    'post', 'page'
                ),
                'content_position' => 'content_manual',
                'button_position' => array(
                    $this->position_name
                ),
                'boombox_activate' => "true",
                'boombox_button_style' => 'button',
                'boombox_template' => '24',
                'boombox_more_button_icon' => 'plus',
                'boombox_share_button_icon' => 'share-alt',
                'boombox_share_button_counter' => 'hidden',
                'boombox_counter_pos' => 'hidden',
                'boombox_total_counter_pos' => 'leftbigicon',
                'boombox_fullwidth_align' => 'left',
                'boombox_fullwidth_share_buttons_columns' => 1,
                'boombox_networks' => array(
                    'facebook',
                    'twitter'
                ),
                'boombox_button_pos' => 'center',
                'boombox_more_button_func' => 1,
                'boombox_share_button_func' => 4,
                'boombox_button_width' => 'flex',
                'boombox_more_button_func' => 4
            );

            if( boombox_is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
                $boombox_options[ 'display_in_types' ][] = 'product';
            }

            $options = array_merge( $options, $boombox_options );

            return $options;
        }

        public function remove_mashshare_meta_tags( $meta_html ) {
            $meta_html = '';

            return $meta_html;
        }

    }

}

function essb_active_oninstall() {
    $bessb = Boombox_Essb::get_instance();
    $bessb->install();
}

if( boombox_is_plugin_active( 'easy-social-share-buttons3/easy-social-share-buttons3.php' ) ) {
    Boombox_Essb::get_instance();
}